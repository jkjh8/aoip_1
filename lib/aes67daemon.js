...
const _aes67Cfg = getSystemConfig().aes67 ?? {};
const DAEMON_URL   = _aes67Cfg.url        ?? 'http://127.0.0.1:8080';
const SERVICE      = _aes67Cfg.service    ?? 'aes67-daemon.service';
const STATUS_FILE  = _aes67Cfg.statusFile ?? '/home/kjh/aes67/status.json';
const DAEMON_CONF  = _aes67Cfg.daemonConf ?? '/home/kjh/aes67/daemon.conf';
...
async function _loadStatusFile() {
  try {
    ...
    if (Array.isArray(data.sources)) {
      ...
      logger.debug('[aes67] status.json sources loaded');
    }
  } catch (e) {
    logger.debug('[aes67] status.json read failed: %s', e.message);
  }
  // sinks는 status.json이 최신 상태가 아닐 수 있으므로 항상 REST API 사용
  try {
...
function _parseDaemonLine(line) {
  ...
  // PTP status change — debounced fetch (PTP can flip multiple times in rapid succession)
  const ptpMatch = line.match(/new PTP clock status (\w+)/);
  if (ptpMatch) {
    const statusText = ptpMatch[1];
    clearTimeout(_ptpTimer);
    _ptpTimer = setTimeout(() => {
      api.get('/api/ptp/status').then(st => {
        _ptpStatus = st;
        daemonEvents.emit('ptp:changed', _ptpStatus);
      }).catch(() => {
        _ptpStatus = { status: statusText };
        daemonEvents.emit('ptp:changed', _ptpStatus);
      });
    }, 500);
    return;
...
export function startDaemonLogForwarder() {
  ...
  const _NOISY = [
    ...
    'browser:: SAP source',          // SAP add/remove floods suppressed from log
    'browser:: removing SAP source',
  ];
  ...
  _journalProc.stdout.on('data', (d) => {
    ...
    while ((nl = _buf.indexOf('\n')) >= 0) {
      ...
      if (!line) continue;
      _parseDaemonLine(line);  // always parse (triggers debounced fetches)
      if (_NOISY.some(p => line.includes(p))) continue;  // suppress noisy logs only
...
// ─── 네트워크 정보 갱신 ──────────────────────────────────
export async function refreshDaemonNetworkConf(iface = _aes67Cfg.interface ?? 'eth0') {
  let conf;
  try {
