...
export const getPtpStatus = async () => _ptpStatus ?? (_ptpStatus = await api.get('/api/ptp/status'));
...
export async function fetchSinkStats(id) {
  ...
  const [st, ptp] = await Promise.allSettled([
    api.get(`/api/sink/status/${id}`),
    api.get('/api/ptp/status'),
  ]);

  if (st.status === 'fulfilled') {
    const f = st.value.sink_flags ?? {};
    if (s.last_flags) {
  ...
  if (ptp.status === 'fulfilled' && ptp.value.jitter != null)
    s.ptp_jitter_ns = ptp.value.jitter;
