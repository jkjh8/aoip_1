...
function startOneBridge(cfg, attemptsLeft = MAX_RETRIES) {
  return new Promise((resolve) => {
    ...
    console.log('[bridges] Starting %s (%s %s) attempt=%d → log: %s',
    ...
    const stableTimer = setTimeout(() => {
      if (settled) return;
      settled = true;
      console.log('[bridges] %s is running (pid=%d)', cfg.name, proc.pid);
    ...
    proc.on('exit', (code, signal) => {
      if (!settled) {
        ...
        settled = true;
        logStream.end();
        console.warn('[bridges] %s exited early (code=%s signal=%s), attempts left=%d',
          cfg.name, code, signal, attemptsLeft - 1);

        if (attemptsLeft > 1) {
          setTimeout(() => resolve(startOneBridge(cfg, attemptsLeft - 1)), RETRY_DELAY_MS);
        } else {
          console.warn('[bridges] %s: all retries exhausted — skipping device', cfg.name);
          resolve(false);
        }
      } else {
        // ── 안정 실행 중 종료 → 자동 재기동 ──
        ...
        if (signal !== 'SIGTERM') {
          console.log('[bridges] %s will restart in 5s', cfg.name);
