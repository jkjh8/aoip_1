...
export async function setUsbGadgetEnabled(enable) {
  ...
  if (enable) {
    ...
    // 프로세스는 유지하고 JACK 연결만 끊기
    try {
      const { getConnections, disconnect } = await import('./jack.js');
      const usbNames = new Set(usbCfgs.map(c => c.name));
      const connections = await getConnections();
      for (const { port, connections: conns } of connections) {
        for (const conn of conns) {
          if (usbNames.has(port.split(':')[0]) || usbNames.has(conn.split(':')[0])) {
            try { await disconnect(port, conn); } catch { /* 이미 끊겨있으면 무시 */ }
          }
        }
      }
    } catch (e) {
      logger.warn('[bridges] usb_gadget disconnect: %s', e.message);
    }
    return true;
  }
}
