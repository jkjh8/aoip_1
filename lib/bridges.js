...
function _stopUsbBridges() {
  ...
  for (const cfg of (config.bridges ?? []).filter(b => b.usb_gadget)) {
    ...
    if (entry?.registered) {
      ...
      // 삭제하지 않고 stopped 상태로 유지 — 재연결 시 bridge start로 재사용
...
export function restartBridge(name) {
  ...
  const sn = entry.safeName ?? _safeName(name);
  sendToEngine(`bridge stop ${sn}`);
  setTimeout(() => sendToEngine(`bridge start ${sn}`), 1000);
...
export async function startupBridges(config) {
...
export async function reregisterBridges(config) {
