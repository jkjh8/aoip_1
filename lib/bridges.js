...
const ALSA_TYPES  = new Set(['audio_in', 'audio_out']);
...
function buildArgs(cfg) {
  if (ALSA_TYPES.has(cfg.type)) {
    ...
    if (cfg.ring_frames != null) args.push(String(cfg.ring_frames));
    return args;
  }
  /* zita-a2j / zita-j2a */
  const args = [
...
function startOneBridge(cfg) {
  ...
  return new Promise((resolve) => {
    ...
    if (ALSA_TYPES.has(cfg.type)) {
      /* audio_in/audio_out: stdout의 ready 신호로 기동 확인 */
      let buf = '';
...
export async function killOrphanBridges() {
  const binaries = ['zita_in', 'zita_out', 'zita-a2j', 'zita-j2a', 'audio_in', 'audio_out'];
...
/* audio_in/audio_out은 자체 재오픈 루프가 있어서 disconnect 시 죽이지 않음 */
