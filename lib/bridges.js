...
function _registerBridge(cfg, chStart) {
  ...
  const cmd = `bridge ${sub} ${cfg.name} ${cfg.device} ${cfg.rate ?? 48000} ${cfg.period ?? 512} ${cfg.periods ?? 8} ${cfg.channels ?? 2} ${chStart}`;
  logger.info('[bridges] registering %s: %s', cfg.name, cmd);
  sendToEngine(cmd);
  _bridgeMap.set(cfg.name, { cfg, registered: true });
}
