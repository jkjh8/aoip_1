...
const USB_BOOT_DELAY_MS  = 6000; // 부팅 시 USB 오디오 클럭 안정화 대기
const USB_CONNECT_DELAY_MS = 3000; // 핫플러그 연결 시 안정화 대기
...
function startOneBridge(cfg, attemptsLeft = MAX_RETRIES) {
  ...
  return new Promise((resolve) => {
    ...
    const proc = spawn('taskset', ['-c', '3', 'chrt', '-f', '80', binary, ...args], {
...
export async function startBridges(bridges) {
  await Promise.all((bridges ?? []).map(cfg => {
    ...
    if (cfg.type === 'ravenna') {
      ...
      const tasks = [];
      if (!bridgeMap.has(inCfg.name) && !pendingSet.has(inCfg.name))
        tasks.push(startOneBridge(inCfg));
      else logger.info('[bridges] %s is already running, skipping', inCfg.name);
      if (!bridgeMap.has(outCfg.name) && !pendingSet.has(outCfg.name))
        tasks.push(startOneBridge(outCfg));
      else logger.info('[bridges] %s is already running, skipping', outCfg.name);
...
export function startUsbGadgetWatcher() {
  ...
  let connectTimer  = null; // 연결 안정화 딜레이 타이머

  const scheduleUsbStart = (delayMs) => {
    if (connectTimer) return; // 이미 대기 중
  ...
  if (prevConnected) {
    logger.info('[bridges] USB host already connected at boot — starting bridges in %dms', USB_BOOT_DELAY_MS);
    scheduleUsbStart(USB_BOOT_DELAY_MS);
  } else {
  ...
  setInterval(() => {
    ...
    if (connected !== prevConnected) {
      ...
      if (connected) {
        logger.info('[bridges] USB host connected — starting bridges in %dms', USB_CONNECT_DELAY_MS);
        scheduleUsbStart(USB_CONNECT_DELAY_MS);
        setTimeout(() => _applyUsbRouting(true), USB_CONNECT_DELAY_MS + STARTUP_GRACE_MS + 500);
      } else {
    ...
    if (connected && !connectTimer) _startUsbBridges();
  }, UDC_POLL_MS);
}
