...
import { startAllMeters, stopAllMeters, getLevel } from './meter.js';
...
function defaultEq() {
  return [
    { enabled: false, freq:  100, gain: 0, q: 0.7, type: 'peak' },
    { enabled: false, freq:  500, gain: 0, q: 0.7, type: 'peak' },
    { enabled: false, freq: 2000, gain: 0, q: 0.7, type: 'peak' },
    { enabled: false, freq: 8000, gain: 0, q: 0.7, type: 'peak' }
  ];
}
...
function defaultOutput(id, label, sinkPort) {
  ...
  return {
    ...
    gain: 100, muted: false,
    dsp: { eq: defaultEq(), limiter: { enabled: false, threshold: -6, release: 50 } }
  };
}
...
export function getChannels(connections = []) {
  ...
  return {
    inputs:  state.inputs.map(ch => ({ ...ch, level: getLevel(ch.jackPort) })),
    outputs: state.outputs.map(ch => {
      const srcs  = srcsByDst.get(ch.jackPort) ?? [];
      const level = srcs.length > 0 ? Math.max(...srcs.map(p => getLevel(p))) : -100;
      return { ...ch, level };
    })
  };
}
...
export function startMeters() {
  // capture 포트(JACK output) + alsa_in 브릿지 포트만 미터 가능
  // playback 포트(JACK input)는 meter 연결 불가 → getChannels()에서 소스 레벨로 대체
  startAllMeters(state.inputs.map(ch => ch.jackPort));
}
