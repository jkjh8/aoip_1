...
/* ch 1..RAVENNA_COUNT → "gainer", 나머지 → "mixer" */
function dspClientOf(id)  { return id <= RAVENNA_COUNT ? 'gainer' : 'mixer'; }
function dspLocalId(id)   { return id <= RAVENNA_COUNT ? id : id - RAVENNA_COUNT; }
function inputJackPort(id)  { return `${dspClientOf(id)}:out_${dspLocalId(id)}`; }
function outputJackPort(id) { return `${dspClientOf(id)}:sin_${dspLocalId(id)}`; }
export function getDspClientOf(id)  { return dspClientOf(id); }
export function getDspLocalId(id)   { return dspLocalId(id); }
...
function addBridge(b) {
  ...
  // isRavenna: 채널 순서를 마지막에 배치 (DSP 고번호)
  // needsSuffix: 포트명에 _in/_out 접미사가 필요한 경우 (ravenna 타입만 해당)
  const isRavenna   = b.type === 'ravenna' || b.aes67 === true;
  const needsSuffix = b.type === 'ravenna'; // aes67은 이미 이름에 _in/_out 포함
  const isInput     = b.type === 'zita_in'  || b.type === 'ravenna';
  const isOutput    = b.type === 'zita_out' || b.type === 'ravenna';
