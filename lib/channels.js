...
for (const s of (config.rtp_streams ?? [])) {
  ...
  if (s.type === 'rtp_in') {
    for (let ch = 1; ch <= chCount; ch++)
      rtpInputs.push(defaultInput(nextInId++, `${label} ${ch}`, `${s.client}:out_${ch}`));
  } else if (s.type === 'rtp_out') {
    for (let ch = 1; ch <= chCount; ch++)
      rtpOutputs.push(defaultOutput(nextOutId++, `${label} ${ch}`, `${s.client}:in_${ch}`));
  }
}
