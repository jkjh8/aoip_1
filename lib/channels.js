...
const rtpInputs   = [];
const rtpOutputs  = [];
// RTP 스트림별 ch_start (aoip_engine rtp_in/rtp_out add 명령용)
const _rtpChStart = new Map(); // `${type}:${client}` → ch_start
for (const s of (config.rtp_streams ?? [])) {
  const chCount = s.channels ?? 2;
  const label   = s.name ?? s.client;
  const enabled = s.enabled !== false;
  if (s.type === 'rtp_in') {
    _rtpChStart.set(`in:${s.client}`, nextInId - 1);  // ch_start = first id - 1
    for (let ch = 1; ch <= chCount; ch++) {
      const id = nextInId++;
...
/** aoip_engine의 rtp_in/rtp_out add 명령에 사용할 ch_start 반환 */
export function getRtpChStart(client, dir = 'in') {
  return _rtpChStart.get(`${dir}:${client}`) ?? 0;
}

/**
 * aoip_engine의 bridge add/add_in/add_out 명령에 사용할 ch_start 반환.
 * channels.js의 실제 ID 할당 순서를 기반으로 계산 (RTP 스트림 포함).
 * ch_start = 0-based index = (첫 번째 채널 ID) - 1
 */
...
export function getChannels(connections = []) {
  ...
  return {
    ...
    outputs: state.outputs.map(ch => {
      ...
      const level = getOutLevel(`${dspClientOf(ch.id, 'out')}:sin_${dspLocalId(ch.id, 'out')}`);
      return { ...ch, level };
    })
  };
}
