...
import { getInLevel, getOutLevel } from './gainer.js';
...
for (const b of (config.bridges ?? [])) {
  ...
  if (b.enabled === false) continue;   /* disabled 브리지는 채널 생성 제외 */

  const isInput     = b.type === 'alsa_in'  || b.type === 'zita-a2j';
  const isOutput    = b.type === 'alsa_out' || b.type === 'zita-j2a';
  const isUsbBridge = b.type === 'usb_jack_bridge';

  if (isInput || isUsbBridge) {
    for (let ch = 1; ch <= chCount; ch++) {
      // usb_jack_bridge: USB→JACK 출력 포트가 소스
      const srcPort = isUsbBridge ? `${b.name}:out_${ch}` : `${b.name}:capture_${ch}`;
      bridgeInputs.push(
        defaultInput(nextInId++, `${abbr} ${ch}`, srcPort)
      );
    }
  }
  if (isOutput || isUsbBridge) {
    for (let ch = 1; ch <= chCount; ch++) {
      // usb_jack_bridge: JACK→USB 입력 포트가 싱크
      const sinkPort = isUsbBridge ? `${b.name}:in_${ch}` : `${b.name}:playback_${ch}`;
      bridgeOutputs.push(
        defaultOutput(nextOutId++, `${abbr} ${ch}`, sinkPort)
      );
    }
  }
}

// RTP 스트림 채널 (rtp_recv → JACK 입력 / JACK 출력 → rtp_send)
const rtpInputs = config.rtp ? [
  defaultInput(nextInId++, 'RTP In L', 'rtp_in:out_1'),
  defaultInput(nextInId++, 'RTP In R', 'rtp_in:out_2'),
] : [];
const rtpOutputs = config.rtp ? [
  defaultOutput(nextOutId++, 'RTP Out L', 'rtp_send:in_1'),
  defaultOutput(nextOutId++, 'RTP Out R', 'rtp_send:in_2'),
] : [];
