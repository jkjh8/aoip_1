...
export function _assignInput(id, client) {
  ...
  registerChannelDsp(id, client, localId, 'in')
  return localId
}
...
export function _assignOutput(id, client) {
  ...
  registerChannelDsp(id, client, localId, 'out')
  return localId
}
...
export function makeInput(id, label, srcPort, dspEnabled = false) {
  return { id, label, port: inputJackPort(id), srcPort, gain: 100, muted: false, dspEnabled: Boolean(dspEnabled) }
}

export function makeOutput(id, label, sinkPort, dspEnabled = false) {
  return { id, label, port: outputJackPort(id), sinkPort, gain: 100, muted: false, dspEnabled: Boolean(dspEnabled) }
}

export function defaultInput(id, label, srcPort, dspEnabled = false) {
  inputSrcPorts.push({ id, srcPort })
  return makeInput(id, label, srcPort, dspEnabled)
}

export function defaultOutput(id, label, sinkPort, dspEnabled = false) {
  outputSinkPorts.push({ id, sinkPort })
  return makeOutput(id, label, sinkPort, dspEnabled)
}
