...
import { getInLevel, getOutLevel,
         connect, disconnect, sendGain, sendMute } from '../dsp/index.js'
...
const ravennaInputs = Array.from({ length: ANALOG_COUNT }, (_, i) => {
  ...
  return { ...defaultInput(id, `${baseName} ${id}`, `system:capture_${id}`), source: { type: 'analog', ch: i + 1 }, active: true }
})
const ravennaOutputs = Array.from({ length: ANALOG_COUNT }, (_, i) => {
  const id = i + 1
  _assignOutput(id, 'analog')
  return { ...defaultOutput(id, `${baseName} ${id}`, `system:playback_${id}`), source: { type: 'analog', ch: i + 1 }, active: true }
})
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const srcPort = needsSuffix
        ...
        : b.type === 'audio_in'
          ? `${b.name}:audio_in_${ch}`
          : `${b.name}:capture_${ch}`
      const chan = { ...makeInput(id, `${abbr} ${ch}`, srcPort), source: { type: srcType, name: b.name, ch }, active: false }
      bIns.push(chan)
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const sinkPort = needsSuffix
        ...
        : b.type === 'audio_out'
          ...
          : `${b.name}:playback_${ch}`
      const chan = { ...makeOutput(id, `${abbr} ${ch}`, sinkPort), source: { type: srcType, name: b.name, ch }, active: false }
      bOuts.push(chan)
...
for (const s of (config.rtp_streams ?? [])) {
  ...
  if (s.type === 'rtp_in') {
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const chan = { ...makeInput(id, `${label} ${ch}`, `${s.client}:out_${ch}`), source: { type: 'rtp', name: s.client, ch }, active: false }
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const chan = { ...makeOutput(id, `${label} ${ch}`, `${s.client}:in_src_${ch}`), source: { type: 'rtp', name: s.client, ch }, active: false }
...
if (existsSync(STATE_FILE)) {
  try {
    ...
    state.inputs  = state.inputs.map(def  => { const { id, port, srcPort,  active: _a, ...u } = savedInMap.get(def.port)  ?? {}; return deepMerge(def, u) })
    state.outputs = state.outputs.map(def => { const { id, port, sinkPort, active: _a, ...u } = savedOutMap.get(def.port) ?? {}; return deepMerge(def, u) })
...
function save() {
  ...
  const data = { inputs: state.inputs.map(strip), outputs: state.outputs.map(strip), routing: state.routing }
...
export function addRtpStreamChannels(cfg) {
  ...
  if (cfg.type === 'rtp_in') {
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      const id = nextInId++
      _assignInput(id, 'stream')
      const chan = { ...makeInput(id, `${label} ${ch}`, `${cfg.client}:out_${ch}`), source: { type: 'rtp', name: cfg.client, ch }, active: false }
      if (enabled) inputSrcPorts.push({ id, srcPort: chan.srcPort })
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      const id = nextOutId++
      _assignOutput(id, 'stream')
      const chan = { ...makeOutput(id, `${label} ${ch}`, `${cfg.client}:in_src_${ch}`), source: { type: 'rtp', name: cfg.client, ch }, active: false }
      if (enabled) outputSinkPorts.push({ id, sinkPort: chan.sinkPort })
...
export function getChannels() {
  const serialize = ({ id, label, port, gain, muted, active }, level) => ({ id, label, port, gain, muted, active, level })
...
export function getAllChannelDefs() {
  return {
    inputs:  state.inputs.map(({ id, label, port, active, source }) => ({ id, label, port, active, source })),
    outputs: state.outputs.map(({ id, label, port, active, source }) => ({ id, label, port, active, source })),
  }
}
