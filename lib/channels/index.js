    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const srcPort = needsSuffix
        ...
        : b.type === 'audio_in'
          ? `${b.name}:audio_in_${ch}`
          : `${b.name}:capture_${ch}`
      const chan = { ...makeInput(id, `${abbr} ${ch}`, srcPort, dspEnabled), source: { type: srcType, name: b.name, ch }, active: false }
      bIns.push(chan)
    ...
    for (let ch = 1; ch <= chCount; ch++) {
      ...
      const sinkPort = needsSuffix
        ...
        : b.type === 'audio_out'
          ...
          : `${b.name}:playback_${ch}`
      const chan = { ...makeOutput(id, `${abbr} ${ch}`, sinkPort, dspEnabled), source: { type: srcType, name: b.name, ch }, active: false }
      bOuts.push(chan)
