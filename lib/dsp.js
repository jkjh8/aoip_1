...
export function startDsp(n_in, n_out) {
  ...
  proc = spawn('taskset', ['-c', '2', 'chrt', '-f', '85', BINARY, String(n_in), String(n_out)], {
  ...
  proc.on('exit', (code, signal) => {
    ...
    if (signal !== 'SIGTERM') {
      ...
      execFile('jack_lsp', [], { timeout: 1000 }, (err) => {
        ...
        setTimeout(() => { restarting = false; startDsp(saved_n_in, saved_n_out); }, 3000);
      });
    }
  });
...
export function sendHpf(ch, params) {
  ...
  if (params.slope !== undefined)
    write(`hpf ${dir} ${ch} slope ${params.slope}`);
  if (params.freq !== undefined)
    write(`hpf ${dir} ${ch} freq ${params.freq}`);
  if (params.enabled !== undefined)
    write(`hpf ${dir} ${ch} enable ${params.enabled ? '1' : '0'}`);
}

const BAND_TYPE_MAP = {
  peak: 'peak', low_shelf: 'loshelf', high_shelf: 'hishelf', lp: 'lp', hp: 'hp',
  loshelf: 'loshelf', hishelf: 'hishelf',
};
 ...
 * @param {{ enabled?: boolean, b0?: number, b1?: number, b2?: number, a1?: number, a2?: number, freq?: number, gain?: number, q?: number, bandType?: string }} params
 */
export function sendEqCoeffs(dir, ch, band, params) {
  if (params.enabled !== undefined)
    write(`eq ${dir} ${ch} ${band} enable ${params.enabled ? '1' : '0'}`);
  if (params.b0 != null) {
    // 프론트엔드가 계산한 biquad 계수 직접 전송
    write(`eq ${dir} ${ch} ${band} coeffs ${params.b0} ${params.b1} ${params.b2} ${params.a1} ${params.a2}`);
  } else if (params.freq != null) {
    // 계수가 null인 경우 — dsp_engine이 freq/gain/q/type으로 직접 계산하도록 전송
    const type = BAND_TYPE_MAP[params.bandType] ?? 'peak';
    write(`eq ${dir} ${ch} ${band} type ${type}`);
    write(`eq ${dir} ${ch} ${band} freq ${params.freq}`);
    write(`eq ${dir} ${ch} ${band} gain ${params.gain ?? 0}`);
    write(`eq ${dir} ${ch} ${band} q ${params.q ?? 0.7}`);
  }
}
