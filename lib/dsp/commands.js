...
export function sendEqEnabled(dir, globalId, enabled) {
  resolveChannel(globalId)
  sendToEngine(`eq ${dir} ${globalId} global ${enabled ? '1' : '0'}`)
}
...
export function sendAllDsp({ inputs, outputs }) {
  for (const ch of inputs) {
    ...
    sendEqEnabled('in', ch.id, dsp?.eqEnabled !== false)
  ...
  for (const ch of outputs) {
    ...
    const { dsp } = ch
    sendEqEnabled('out', ch.id, dsp?.eqEnabled !== false)
