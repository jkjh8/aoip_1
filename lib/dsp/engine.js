...
const ENGINE_BIN = resolve(__dirname, '../../scripts/aoip_engine')
...
function _parseLine(line) {
  ...
  if (parts[0] === 'clk2') {
    ...
    const src       = kv.dsp_src ?? '-'
    const sign = (v) => v >= 0 ? '+' : ''
    logger.info(`[dsp] hw:aoip↔RAVENNA: aoip=${aoip.toFixed(3)}Hz  ravenna=${rav.toFixed(3)}Hz  drift=${sign(ppm)}${ppm.toFixed(3)}ppm  over ${elapsed.toFixed(0)}s  dsp=${period}fr(${tickMs.toFixed(3)}ms)@${src}`)
    return
  }
...
function _launch() {
  ...
  const child = spawn('chrt', ['-f', '82', 'taskset', '-c', '2,3', ENGINE_BIN, '--name', 'main'], {
  ...
  child.stderr.on('data', (d) => {
    ...
    while ((nl = _errBuf.indexOf('\n')) >= 0) {
      ...
      if (line) logger.info('[engine] %s', line)
    }
  })
...
export function stopEngine() {
  if (_engineProcess) {
    _engineProcess.removeAllListeners('exit')
