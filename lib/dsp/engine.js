...
function _parseLine(line) {
  ...
  if (parts[0] === 'clk2') {
    ...
    const sign = (v) => v >= 0 ? '+' : ''
    logger.info(`[dsp] hw:aoip↔RAVENNA: aoip=${aoip.toFixed(3)}Hz  ravenna=${rav.toFixed(3)}Hz  drift=${sign(ppm)}${ppm.toFixed(3)}ppm  over ${elapsed.toFixed(0)}s  dsp=${period}fr(${tickMs.toFixed(3)}ms)`)
    return
  }
