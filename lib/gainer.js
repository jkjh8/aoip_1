  ...
  proc.stdout.on('data', d => {
    ...
    while ((nl = g_stdoutBuf.indexOf('\n')) >= 0) {
      ...
      /* lm out <ch> <pre_db> <post_db> */
        ...
        process.stderr.write('[dsp] ' + line + '\n');
      }
    }
  });
  proc.stderr.on('data', d => process.stderr.write('[dsp] ' + d));

  proc.on('exit', (code, signal) => {
    console.log('[dsp] exited code=%s signal=%s', code, signal);
    proc = null;
    if (signal !== 'SIGTERM') {
      console.log('[dsp] restarting in 3s...');
      setTimeout(() => startGainer(saved_n_in, saved_n_out), 3000);
    }
  });

  proc.on('error', err => {
    console.error('[dsp] process error:', err.message);
    proc = null;
  });

  console.log('[dsp] started (in=%d out=%d)', n_in, n_out);
}
