...
function saveAudioConfig(updatedCfg) {
  const raw = getConfig()
  const idx = (raw.rtp_streams ?? []).findIndex(
    (s) => s.client === updatedCfg.client
  )
  if (idx >= 0) {
    const keep = [
      'type',
...
function _waitForFlag(getFlag, label, timeoutMs) {
  if (getFlag()) return Promise.resolve()
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => reject(new Error(`${label} ready timeout`)), timeoutMs)
    const poll = setInterval(() => {
      if (getFlag()) { clearInterval(poll); clearTimeout(t); resolve() }
    }, 100)
  })
}
...
function _parseRxLine(line) {
  ...
  if (line.startsWith('[rtp_recv] ready')) {
    rxReady = true
    logger.info(
      '[gst] rx started  rtp_recv + gst udpsrc port=%d codec=%s',
      rxPort,
      rxStats.codec
    )
    return
...
const RTP_IN_STATS_RE = /codec=(.+?)\s+bufMs=(\d+)\s+packets=(\d+)\s+drops=(\d+)\s+srcIp=(\S+)\s+srcPort=(\d+)\s+bitrateKbps=(\d+)/

function _applyRtpInStats(stats, line) {
...
async function _launchRtpIn(cfg) {
  ...
  const protoMode =
    cfg.protocol === 'pcm' ? 'pcm' : cfg.protocol === 'rtp' ? 'rtp' : 'raw'
  const address =
    cfg.address && cfg.address !== '0.0.0.0' ? cfg.address : '0.0.0.0'
  ...
  /* rtp_recv shm 모드 기동 */
  const args = [
    String(cfg.port ?? 10001),
    String(ch),
    protoMode,
    key,
    String(cfg.bufferMs ?? 100),
    String(cfg.sampleRate ?? 48000),
  ...
  logger.info(
    '[gst] starting rtp_recv (shm) %s  port=%d addr=%s shm=%s',
    key,
    cfg.port ?? 10001,
    address,
    shmName
    ...
    d.toString()
      ...
      .forEach((line) => {
        if (line.startsWith('[rtp_recv] ready')) {
          ...
          _applyRtpInStats(inst.stats, line)
          ...
          if (parts.length >= 3) {
            ...
            logger.info(
              '[gst] rtp_in %s auto-codec: %s %sHz',
              key,
              inst.cfg.rtpEncoding,
              inst.cfg.sampleRate
            )
          }
        } else {
  ...
  proc.on('exit', (code, signal) => {
    ...
    setTimeout(() => {
      if (inst.shouldRun) _launchRtpIn(inst.cfg)
    }, delay)
  })
...
function _launchRtpSend(cfg, shmName, inst) {
  ...
  const bitrateArg = String(cfg.bitrate ?? 320)
  const args = [
    String(cfg.channels ?? 2),
    key,
    proto,
    String(cfg.sampleRate ?? 0),
    'shm',
    shmName,
    codecArg,
    bitrateArg
  ...
  logger.info(
    '[gst] starting rtp_send (pipe) %s: %s %s',
    key,
    bin,
    args.join(' ')
  )
  ...
  proc.stdout.on('data', (d) => {
    ...
    while ((nl = buf.indexOf('\n')) >= 0) {
      ...
      if (line.startsWith('[rtp_send] ready')) {
        ...
        const m = line.match(
          /targets=(\d+)\s+codec=(\S+)\s+bitrateKbps=(\d+)\s+bytesSent=(\d+)/
        )
        if (m) {
          inst.stats = {
            targets: Number(m[1]),
  ...
  proc.on('exit', (code, signal) => {
    ...
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0) {
      // SHM은 aoip_engine이 소유·유지하므로 remove/add 없이 rtp_send만 재시작
      setTimeout(() => {
        if (!inst.shouldRun) return
        try { execSync(`pkill -f "rtp_send.*${key}"`, { stdio: 'ignore' }) } catch { /* ignore */ }
        _launchRtpSend(inst.cfg, shmName, inst)
      }, 200)
    }
  })
  proc.on('error', (err) => {
...
export function addRtpOutTarget(client, host, port) {
  ...
  if (!inst || inst.cfg.type !== 'rtp_out')
    throw new Error(`rtp_out ${client} not found`)

  /* 다른 인스턴스에서 같은 주소 사용 중이면 제거 */
  for (const [key, other] of streamInstances) {
    if (key === client || other.cfg.type !== 'rtp_out') continue
    const dup = (other.cfg.targets ?? []).find(
      (t) => t.host === host && t.port === port
    )
    if (dup) {
      other.cfg.targets = other.cfg.targets.filter(
        (t) => !(t.host === host && t.port === port)
      )
      if (other.proc && !other.proc.killed)
        other.proc.stdin.write(`remove ${host} ${port}\n`)
      logger.info(
        '[gst] rtp_out %s: removed duplicate target %s:%d (moved to %s)',
        key,
        host,
        port,
        client
  ...
  if (inst.proc && !inst.proc.killed)
    inst.proc.stdin.write(`add ${host} ${port}\n`)
  saveAudioConfig(inst.cfg)
...
export function removeRtpOutTarget(client, host, port) {
  ...
  if (!inst || inst.cfg.type !== 'rtp_out')
    throw new Error(`rtp_out ${client} not found`)
  inst.cfg.targets = (inst.cfg.targets ?? []).filter(
    (t) => !(t.host === host && t.port === port)
  )
  if (inst.proc && !inst.proc.killed)
    inst.proc.stdin.write(`remove ${host} ${port}\n`)
  saveAudioConfig(inst.cfg)
  logger.info('[gst] rtp_out %s remove target %s:%d', client, host, port)
}

/** rtp_out 코덱 변경 (mp3 | raw 만 지원) */
export function setRtpOutCodec(client, codec, bitrate) {
  ...
  if (inst.proc && !inst.proc.killed)
    inst.proc.stdin.write(
      `codec ${inst.cfg.codec} ${inst.cfg.bitrate ?? 320}\n`
    )
  saveAudioConfig(inst.cfg)
  logger.info(
    '[gst] rtp_out %s codec %s %d',
    client,
    inst.cfg.codec,
    inst.cfg.bitrate ?? 320
  )
}
...
export function startRtpStreams(streams) {
  ...
  for (const cfg of streams ?? []) {
    ...
    try {
      execSync(`pkill -f "rtp_recv.*${cfg.client}|rtp_send.*${cfg.client}"`, {
        stdio: 'ignore'
      })
    } catch {
      /* 없으면 무시 */
...
export function updateRtpInConfig(client, updates) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_in')
    throw new Error(`rtp_in ${client} not found`)
  Object.assign(inst.cfg, updates)
  saveAudioConfig(inst.cfg)
  logger.info(
    '[gst] rtp_in %s config updated (restart required to apply)',
    client
  )
}
/** rtp_out 설정 변경 (protocol, sampleRate, channels) — 프로세스 재시작 필요 */
export function updateRtpOutConfig(client, updates) {
  ...
  allowed.forEach((k) => {
    if (updates[k] !== undefined) inst.cfg[k] = updates[k]
  })
  saveAudioConfig(inst.cfg)
...
export function waitForRtpStreamsReady(timeoutMs = 8000) {
  return new Promise((resolve, reject) => {
    ...
    const t = setTimeout(
      () => reject(new Error('rtp_streams ready timeout')),
      timeoutMs
    )
    const poll = setInterval(() => {
      const allReady = Array.from(streamInstances.values()).every(
        (i) => i.ready
      )
      if (allReady) {
        clearInterval(poll)
