...
function _launchRtpSend(cfg, shmName, inst) {
  ...
  const args = [String(cfg.channels ?? 2), key, proto,
                String(cfg.sampleRate ?? 0), 'shm', shmName, codecArg, bitrateArg];
  ...
  proc.on('exit', (code, signal) => {
    ...
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0) {
      setTimeout(() => {
        ...
        // SHM 재생성: SIGBUS 등 비정상 종료 시 stale SHM 제거 후 재등록
        sendToEngine(`rtp_out remove ${key}`);
        const ch = inst.cfg.channels ?? 2;
        const chStart = getRtpChStart(key, 'out');
        setTimeout(() => {
          if (!inst.shouldRun) return;
          sendToEngine(`rtp_out add ${key} ${shmName} ${ch} ${chStart}`);
          setTimeout(() => { if (inst.shouldRun) _launchRtpSend(inst.cfg, shmName, inst); }, 300);
        }, 200);
      }, 2000);
    }
  });
