...
function _reconnectRtpIn(key) {
  if (!_startupComplete) return;  // startup이 srcPorts 연결을 담당
  const srcPorts = getInputSrcPorts();
  setTimeout(() => {
    for (const { id, srcPort } of srcPorts) {
      if (!srcPort.startsWith(`${key}:`)) continue;
      connect(srcPort, `${getDspClientOf(id)}:in_${getDspLocalId(id)}`).catch(e =>
        logger.warn('[gst] rtp_in %s reconnect %s→%s:in_%d: %s', key, srcPort, getDspClientOf(id), getDspLocalId(id), e.message)
      );
    }
...
function _reconnectRtpOut(key) {
  ...
  _reconnectRtpOutTimers.set(key, setTimeout(() => {
    ...
    for (let i = 0; i < assigned.length; i++) {
      ...
      connect(`${getDspClientOf(id)}:sout_${getDspLocalId(id)}`, actualPort).catch(e =>
        logger.warn('[gst] rtp_out %s reconnect %s:sout_%d→%s: %s', key, getDspClientOf(id), getDspLocalId(id), actualPort, e.message)
      );
    }
...
function _launchRtpOut(cfg) {
  ...
  const proc = spawn('taskset', ['-c', '2', 'chrt', '-f', '75', bin, ...args], { stdio: ['pipe', 'pipe', 'pipe'], detached: false });
  inst.proc = proc;
