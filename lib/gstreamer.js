...
export function startTxPipeline(targets, { bitrate = 320 } = {}) {
  if (txProcess) {
    logger.info('[gst] stopping existing tx before rebuild');
...
export function stopTxPipeline() {
  if (!txProcess) return;
  logger.info('[gst] stopping tx (pid %d)', txProcess.pid);
  txProcess.kill('SIGTERM');
  txProcess = null;
  txTargets = [];
}
...
function _spawn(name, cmd, args, onStart, onExit) {
  ...
  logger.info('[gst] starting %s: taskset -c 2,3 %s %s', name, cmd, args.join(' '));
  logger.info('[gst] log → %s', logPath);

  const proc = spawn('taskset', ['-c', '2,3', cmd, ...args], {
  ...
  proc.on('exit', (code, signal) => {
    logger.info('[gst] %s exited (code=%s signal=%s)', name, code, signal);
    onExit();
  });

  proc.on('error', (err) => {
    logger.error('[gst] %s error: %s', name, err.message);
    onExit();
  });
