...
/** rtp_in/rtp_out 재연결은 aoip_engine 내부에서 처리 — no-op */
function _reconnectRtpIn(key)  { /* no-op: aoip_engine handles FIFO reconnect */ }
function _reconnectRtpOut(key) { /* no-op */ }

/** FIFO 생성 헬퍼 (이미 존재하면 무시) */
function _mkfifo(path) {
  return new Promise(resolve =>
    execFile('mkfifo', [path], () => resolve())  // 이미 존재해도 무시
  );
}
...
async function _launchRtpIn(cfg) {
  ...
  const fifoPath  = `/tmp/rtp_in_${key}`;
  ...
  /* FIFO 생성 → aoip_engine에 rtp_in 등록 (reader 시작) */
  await _mkfifo(fifoPath);
  sendToEngine(`rtp_in add ${key} ${fifoPath} ${ch} ${chStart}`);

  /* rtp_recv pipe 모드 기동 (writer) */
  const args = [
  ...
  logger.info('[gst] starting rtp_recv (pipe) %s  port=%d addr=%s fifo=%s', key, cfg.port ?? 10001, address, fifoPath);
...
async function _launchRtpOut(cfg) {
  const key      = cfg.client;
  const fifoPath = `/tmp/rtp_out_${key}`;
  const ch       = cfg.channels ?? 2;
  ...
  /* FIFO 생성 → aoip_engine에 rtp_out 등록 (writer 시작, reader 대기) */
  await _mkfifo(fifoPath);
  sendToEngine(`rtp_out add ${key} ${fifoPath} ${ch} ${chStart}`);

  /* rtp_send 기동 (pipe reader) */
...
function _launchRtpSend(cfg, fifoPath, inst) {
  ...
  proc.stdout.on('data', d => {
    ...
    while ((nl = buf.indexOf('\n')) >= 0) {
      ...
      if (line.startsWith('[rtp_send] ready')) {
        inst.ready = true;
        logger.info('[gst] rtp_out %s ready (pipe mode)', key);
  ...
  proc.on('exit', (code, signal) => {
    ...
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0) {
      setTimeout(() => {
        ...
        setTimeout(() => { if (inst.shouldRun) _launchRtpSend(inst.cfg, fifoPath, inst); }, 500);
      }, 2000);
    }
  });
  proc.on('error', err => {
