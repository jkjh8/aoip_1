...
export function notifyRtpStartupComplete() {
...
export function waitForRxReady(timeoutMs = 8000) {
  ...
  return new Promise((resolve, reject) => {
    const t = setTimeout(
      () => reject(new Error('rtp_recv ready timeout')),
      timeoutMs
    )
...
export function waitForTxReady(timeoutMs = 8000) {
  ...
  return new Promise((resolve, reject) => {
    const t = setTimeout(
      () => reject(new Error('rtp_send ready timeout')),
      timeoutMs
    )
    const poll = setInterval(() => {
      if (txReady) {
...
export function updateRtpOutConfig(client, updates) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_out') throw new Error()
