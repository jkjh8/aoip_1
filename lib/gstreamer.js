...
import { getInputSrcPorts, getOutputSinkPorts, getDspClientOf, getDspLocalId } from './channels.js';
...
function _reconnectRtpIn(key) {
  ...
  setTimeout(() => {
    for (const { id, srcPort } of srcPorts) {
      ...
      connect(srcPort, `${getDspClientOf(id,'in')}:in_${getDspLocalId(id,'in')}`).catch(e =>
        logger.warn('[gst] rtp_in %s reconnect %s→dsp:in: %s', key, srcPort, e.message)
      );
    }
...
function _reconnectRtpOut(key) {
  if (!_startupComplete) return;  // startup이 sinkPorts 연결을 담당
  ...
  _reconnectRtpOutTimers.set(key, setTimeout(() => {
    ...
    for (let i = 0; i < assigned.length; i++) {
      ...
      connect(`${getDspClientOf(id,'out')}:sout_${getDspLocalId(id,'out')}`, actualPort).catch(e =>
        logger.warn('[gst] rtp_out %s reconnect dsp:sout→%s: %s', key, actualPort, e.message)
      );
    }
...
function _launchRtpIn(cfg) {
  ...
  const fifoPath   = `/tmp/rtp_pipe_${key}_in`;
  const bridgeBin  = join(SCRIPTS, 'jack_pipe_in');
  const bridgeArgs = [key, String(cfg.channels ?? 2), fifoPath];
  ...
  /* jack_pipe_in: JACK RT 클라이언트 (CPU 3, RT75) */
  const bridgeProc = spawn('taskset', ['-c', '3', 'chrt', '-f', '75', bridgeBin, ...bridgeArgs],
    { stdio: ['ignore', 'pipe', 'pipe'], detached: false });
  inst.bridgeProc = bridgeProc;
  ...
  bridgeProc.stdout.on('data', d => {
    ...
    while ((nl = bridgeBuf.indexOf('\n')) >= 0) {
      ...
      if (!line) continue;
      if (line.startsWith('[jack_pipe_in] fifo_ready')) {
        _launchRtpRecv(cfg, fifoPath, inst);
      } else if (line.startsWith('[jack_pipe_in] ready')) {
        inst.ready = true;
        ...
        if (inst.ready) _reconnectRtpIn(key);
      } else {
        logger.debug('[jack_pipe_in:%s] %s', key, line);
      }
    }
  });
  bridgeProc.stderr.on('data', d =>
    d.toString().split('\n').filter(Boolean)
      .forEach(l => logger.debug('[jack_pipe_in:%s] %s', key, l))
  );
  bridgeProc.on('exit', (code, signal) => {
    logger.info('[jack_pipe_in:%s] exited code=%s signal=%s', key, code, signal);
    inst.bridgeProc = null; inst.ready = false; inst.ports = [];
    if (inst.proc && !inst.proc.killed) inst.proc.kill('SIGTERM');
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0) {
      setTimeout(() => {
        if (!inst.shouldRun) return;
        try { execSync(`pkill -f "jack_pipe_in.*${key}"`, { stdio: 'ignore' }); } catch { /* ignore */ }
        try { execSync(`pkill -f "rtp_recv.*${key}"`,     { stdio: 'ignore' }); } catch { /* ignore */ }
        setTimeout(() => { if (inst.shouldRun) _launchRtpIn(inst.cfg); }, 500);
      }, 2000);
    }
  });
  bridgeProc.on('error', err => {
    logger.error('[jack_pipe_in:%s] error: %s', key, err.message);
    inst.bridgeProc = null;
  });
}

function _launchRtpRecv(cfg, fifoPath, inst) {
  const key       = cfg.client;
  const bin       = join(SCRIPTS, 'rtp_recv');
  const protoMode = cfg.protocol === 'pcm' ? 'pcm' : cfg.protocol === 'rtp' ? 'rtp' : 'raw';
  const address   = cfg.address && cfg.address !== '0.0.0.0' ? cfg.address : '0.0.0.0';
  ...
  logger.info('[gst] starting rtp_recv (pipe) %s: %s %s', key, bin, args.join(' '));

  const proc = spawn('taskset', ['-c', '1', bin, ...args],
  ...
  proc.stderr.on('data', d => {
    ...
    while ((nl = stderrBuf.indexOf('\n')) >= 0) {
      ...
      if (!line) continue;
      if (line.startsWith('[rtp_recv] auto-codec:')) {
  ...
  proc.on('exit', (code, signal) => {
    ...
    inst.proc = null;
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0 && inst.bridgeProc) {
      if (code === 2) {
        /* AUTO 코덱 재감지: jack_pipe_in 유지하고 rtp_recv만 재시작 */
        setTimeout(() => {
          if (inst.shouldRun && inst.bridgeProc) _launchRtpRecv(inst.cfg, fifoPath, inst);
        }, 200);
      } else {
        setTimeout(() => {
          if (!inst.shouldRun || !inst.bridgeProc) return;
          try { execSync(`pkill -f "rtp_recv.*${key}"`, { stdio: 'ignore' }); } catch { /* ignore */ }
          setTimeout(() => { if (inst.shouldRun && inst.bridgeProc) _launchRtpRecv(inst.cfg, fifoPath, inst); }, 500);
        }, 2000);
      }
    }
  });
...
function _launchRtpOut(cfg) {
  const key      = cfg.client;
  const fifoPath = `/tmp/rtp_pipe_${key}`;
  const bridgeBin = join(SCRIPTS, 'jack_pipe_out');
  const bridgeArgs = [key, String(cfg.channels ?? 2), fifoPath];

  const inst = { proc: null, bridgeProc: null, cfg, ready: false, ports: [], shouldRun: true };
  streamInstances.set(key, inst);

  logger.info('[gst] starting jack_pipe_out %s: %s %s', key, bridgeBin, bridgeArgs.join(' '));

  /* jack_pipe_out: JACK RT 클라이언트 (CPU 3, RT 75) */
  const bridgeProc = spawn('taskset', ['-c', '3', 'chrt', '-f', '75', bridgeBin, ...bridgeArgs],
    { stdio: ['ignore', 'pipe', 'pipe'], detached: false });
  inst.bridgeProc = bridgeProc;
  ...
  bridgeProc.stdout.on('data', d => {
    ...
    while ((nl = bridgeBuf.indexOf('\n')) >= 0) {
      ...
      if (line.startsWith('[jack_pipe_out] fifo_ready')) {
        /* FIFO 준비 완료 → rtp_send 기동 */
        _launchRtpSend(cfg, fifoPath, inst);
      } else if (line.startsWith('[jack_pipe_out] ready')) {
        inst.ready = true;
        ...
        logger.debug('[jack_pipe_out:%s] %s', key, line);
      }
    }
  });
  bridgeProc.stderr.on('data', d =>
    d.toString().split('\n').filter(Boolean)
      .forEach(l => logger.debug('[jack_pipe_out:%s] %s', key, l))
  );
  bridgeProc.on('exit', (code, signal) => {
    logger.info('[jack_pipe_out:%s] exited code=%s signal=%s', key, code, signal);
    inst.bridgeProc = null; inst.ready = false; inst.ports = [];
    /* rtp_send도 함께 종료 */
    ...
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0) {
      setTimeout(() => {
        if (!inst.shouldRun) return;
        try { execSync(`pkill -f "jack_pipe_out.*${key}"`, { stdio: 'ignore' }); } catch { /* ignore */ }
  ...
  proc.on('exit', (code, signal) => {
    ...
    if (inst.shouldRun && signal !== 'SIGTERM' && code !== 0 && inst.bridgeProc) {
      setTimeout(() => {
        if (!inst.shouldRun || !inst.bridgeProc) return;
        try { execSync(`pkill -f "rtp_send.*${key}"`, { stdio: 'ignore' }); } catch { /* ignore */ }
        setTimeout(() => { if (inst.shouldRun && inst.bridgeProc) _launchRtpSend(inst.cfg, fifoPath, inst); }, 500);
      }, 2000);
    }
  });
  proc.on('error', err => {
...
export function stopRtpStream(client) {
  ...
  if (inst.bridgeProc && !inst.bridgeProc.killed)
    inst.bridgeProc.kill('SIGTERM');
}
...
export function stopRtpStreams() {
  for (const [, inst] of streamInstances) {
    ...
    if (inst.bridgeProc) inst.bridgeProc.kill('SIGTERM');
  }
...
export function getRtpStreamStatus() {
  return Array.from(streamInstances.entries()).map(([key, inst]) => ({
    ...
    type:     inst.cfg.type,
    name:     inst.cfg.name,
    running:  inst.bridgeProc
                ? inst.bridgeProc.exitCode === null && !inst.bridgeProc.killed
                : inst.proc !== null && !inst.proc?.killed,
    ready:    inst.ready,
    ports:    [...inst.ports],
    stats:    inst.cfg.type === 'rtp_in' ? { ...inst.stats } : undefined,
  }));
