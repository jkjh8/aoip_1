...
const AUDIO_CFG  = join(__dirname, '../config/audio.json');

function saveAudioConfig(updatedCfg) {
  try {
    const raw = JSON.parse(readFileSync(AUDIO_CFG, 'utf8'));
    ...
    writeFileSync(AUDIO_CFG, JSON.stringify(raw, null, 2));
  } catch (e) {
    logger.warn('[gst] audio.json save failed: %s', e.message);
  }
}
...
function _reconnectRtpOut(key) {
  ...
  setTimeout(() => {
    for (const { id, sinkPort } of sinkPorts) {
      if (!sinkPort.startsWith(`${key}:`)) continue;
      connect(`gainer:sout_${id}`, sinkPort).catch(e =>
        logger.warn('[gst] rtp_out %s reconnect gainer:sout_%d→%s: %s', key, id, sinkPort, e.message)
      );
    }
...
function _launchRtpIn(cfg) {
  ...
  const protoMode = cfg.protocol === 'pcm' ? 'pcm' : 'raw';
  const inst = {
  ...
  const modeDesc = protoMode === 'pcm' ? `pcm S16LE ${cfg.sampleRate ?? 48000}Hz` : 'raw';
  logger.info('[gst] starting rtp_in %s  port=%d ch=%d mode=%s buf=%dms',
    key, cfg.port ?? 10001, cfg.channels ?? 2, modeDesc, cfg.bufferMs ?? 100);
