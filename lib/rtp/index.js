...
export async function startupRtp(config) {
  ...
  const streams = config.rtp_streams.filter(s => s.enabled !== false)
  if (!streams.length) return
  logger.info('[startup] Starting %d RTP streams...', streams.length)
  try { startRtpStreams(streams) } catch (e) { logger.warn('[startup] rtp_streams:', e.message) }
