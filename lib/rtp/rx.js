...
function _parseRxLine(line) {
  ...
  if (line.startsWith('stats ')) {
    const prevSrcIp = rxStats.srcIp
    applyRtpInStats(rxStats, line)
    if (rxStats.srcIp !== prevSrcIp) {
      if (rxStats.srcIp)
        logger.info('[rtp_recv] source connected: %s:%d  codec=%s  %dkbps',
          rxStats.srcIp, rxStats.srcPort, rxStats.codec, rxStats.bitrateKbps)
      else
        logger.info('[rtp_recv] source disconnected')
    }
  }
}
