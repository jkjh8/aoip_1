...
async function _waitForShm(shm, timeoutMs = 5000) {
  const path = `/dev/shm${shm}`
  ...
  while (!existsSync(path)) {
    if (Date.now() >= deadline) {
      logger.warn('[rtp] SHM %s not created after %dms', shm, timeoutMs)
      return
    }
    await new Promise(r => setTimeout(r, 100))
  }
...
/** @type {Map<string, { socket, server, cfg, ready, ports, shouldRun, stats }>} */
        ...
        if (line.startsWith('[rtp_recv] ready')) {
          ...
          if (inst.stats.srcIp !== prevSrcIp) {
            if (inst.stats.srcIp)
              ...
              logger.info('[rtp_recv:%s] source disconnected', key)
          }
...
function _spawnRtpIn(inst) {
  const key = inst.cfg.client
  inst.ready = false

  _startRtpInServer(inst)

  try {
    execSync(`systemctl start aoip-rtp-recv@${key}.service`, { stdio: 'ignore' })
  } catch (err) {
    logger.error('[rtp_recv:%s] failed to start service: %s', key, err.message)
    inst.server?.close(); inst.server = null
    if (inst.shouldRun) setTimeout(() => { if (inst.shouldRun) _spawnRtpIn(inst) }, 2000)
  }
}
...
async function _launchRtpIn(cfg) {
  ...
  await _waitForShm(shm)
  if (inst.shouldRun) _spawnRtpIn(inst)
}
...
function _launchRtpSend(cfg, inst) {
  const key = cfg.client
  logger.info('[gst] starting aoip-rtp-send@%s.service', key)

  _startRtpSendServer(inst)

  try {
    execSync(`systemctl start aoip-rtp-send@${key}.service`, { stdio: 'ignore' })
  } catch (err) {
    logger.error('[rtp_send:%s] failed to start service: %s', key, err.message)
    inst.server?.close(); inst.server = null
    if (inst.shouldRun) setTimeout(() => { if (inst.shouldRun) _launchRtpSend(cfg, inst) }, 2000)
  }
}
...
async function _launchRtpOut(cfg) {
  ...
  await _waitForShm(shm)
  if (inst.shouldRun) _launchRtpSend(cfg, inst)
}
...
export function startRtpStreams(streams) {
  for (const cfg of streams ?? []) {
    ...
    const key = cfg.client
    try { execSync(`systemctl stop aoip-rtp-recv@${key}.service aoip-rtp-send@${key}.service`, { stdio: 'ignore' }) } catch { /* ignore */ }
  }
  for (const cfg of streams ?? []) {
...
export function stopRtpStream(client) {
  ...
  if (inst.cfg.type === 'rtp_in') {
    ...
    try { execSync(`systemctl stop aoip-rtp-recv@${client}.service`, { stdio: 'ignore' }) } catch { /* ignore */ }
    inst.socket?.destroy(); inst.socket = null
    inst.server?.close(); inst.server = null
  }
  if (inst.cfg.type === 'rtp_out') {
    sendToEngine(`rtp_out remove ${client}`)
    try { execSync(`systemctl stop aoip-rtp-send@${client}.service`, { stdio: 'ignore' }) } catch { /* ignore */ }
    inst.socket?.destroy(); inst.socket = null
    inst.server?.close(); inst.server = null
  }
}
...
export async function startRtpStream(client) {
  ...
  if (inst.cfg.type === 'rtp_in') {
    const shm     = shmName(client)
    const ch      = inst.cfg.channels ?? 2
    const chStart = getRtpChStart(client, 'in')
    sendToEngine(`rtp_in add ${client} ${shm} ${ch} ${chStart}`)
    await _waitForShm(shm)
    if (inst.shouldRun) _spawnRtpIn(inst)
    ...
    sendToEngine(`rtp_out add ${client} ${shm} ${ch} ${chStart}`)
    await _waitForShm(shm)
