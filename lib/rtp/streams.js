...
function _spawnRtpIn(inst) {
  ...
  const proc = spawn('chrt', ['-f', '50', bin, ...args], {
  ...
  proc.on('exit', (code, signal) => {
    ...
    // 엔진 재등록 없이 프로세스만 재시작 (SHM 재생성 방지)
    setTimeout(() => { if (inst.shouldRun) _spawnRtpIn(inst) }, delay)
  })
  proc.on('error', (err) => { logger.error('[rtp_recv:%s] error: %s', key, err.message); inst.proc = null })
}

/* ── rtp_in 최초 기동 (엔진 등록 + 프로세스 시작) ───────────────── */
...
export function updateRtpOutConfig(client, updates) {
  ...
  /* codec/bitrate 변경은 실행 중인 프로세스에 즉시 반영 */
  if ((updates.codec !== undefined || updates.bitrate !== undefined) && inst.proc && !inst.proc.killed)
    inst.proc.stdin.write(`codec ${inst.cfg.codec ?? 'mp3'} ${inst.cfg.bitrate ?? 320}\n`)
  saveAudioConfig(inst.cfg)
...
export function startRtpStream(client) {
  ...
  if (inst.cfg.type === 'rtp_in')
    ...
    const shm     = shmName('rtp_out', client)
    const ch      = inst.cfg.channels ?? 2
    const chStart = getRtpChStart(client, 'out')
    // 프로세스가 종료된 상태에서만 여기 도달 → SIGBUS 위험 없음
    // stopRtpStream이 rtp_out remove로 SHM을 삭제했으므로 재등록 필요
