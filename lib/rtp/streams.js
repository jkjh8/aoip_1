...
export function startRtpStream(client) {
  ...
  inst.shouldRun = true
  // 이미 엔진에 등록된 스트림: 프로세스만 재시작 (rtp_out add/rtp_in add 재전송 금지)
  // 재전송 시 엔진이 SHM을 shm_unlink+재생성 → 기존 프로세스 mmap 무효화 → SIGBUS
  if (inst.cfg.type === 'rtp_in')
    _spawnRtpIn(inst)
  else if (inst.cfg.type === 'rtp_out')
    _launchRtpSend(inst.cfg, shmName('rtp_out', client), inst)
}
