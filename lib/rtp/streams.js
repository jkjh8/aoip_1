...
function _spawnRtpIn(inst) {
  ...
  /* RTP recv is now an internal thread in aoip_engine — no separate service */
}
...
export async function startRtpStream(client) {
  ...
  if (inst.cfg.type === 'rtp_in') {
    ...
    await _waitForShm(shm)
    if (inst.shouldRun) _spawnRtpIn(inst)  /* creates socket server; engine connects */
    ...
    await _waitForShm(shm)
    if (inst.shouldRun) _launchRtpSend(inst.cfg, inst)  /* creates socket server */
  }
}
