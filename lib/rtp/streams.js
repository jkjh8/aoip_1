 ...
 * startRtpStreams / stopRtpStream / startRtpStream
...
import { sendToEngine, waitForEngineReady, addEngineRestartListener, getRtpBufStats } from '../dsp/index.js'
import { getRtpChStart, addRtpStreamChannels, removeRtpStreamChannels } from '../channels/index.js'
import { applyRtpInStats, saveAudioConfig, deleteAudioConfig } from './utils.js'
...
function _scheduleRtpInRestart(inst, key, delayMs) {
  ...
  inst._restartTimer = setTimeout(async () => {
    ...
    if (inst.socket && !inst.socket.destroyed) return  // rtp_recv alive — no restart needed
    ...
    sendToEngine(`rtp_in add ${key} ${ch} ${chStart} bufMs=${inst.cfg.bufferMs ?? 100}`)
  }, delayMs)
}
...
function _startRtpInServer(inst) {
  ...
  const server = net.createServer((socket) => {
    ...
    socket.write(
      ...
      (inst.cfg.pt != null ? ` pt=${inst.cfg.pt}` : '') +
      ` floodThreshold=${inst.cfg.floodThreshold ?? 5000}` +
      ` floodBackoffMs=${inst.cfg.floodBackoffMs ?? 500}` +
      ` badPktThreshold=${inst.cfg.badPktThreshold ?? 200}` +
      '\n'
    )
        ...
        if (line.startsWith('[rtp_recv] ready')) {
          ...
          if (inst.stats.srcIp !== prevSrcIp) {
            if (inst.stats.srcIp) {
              ...
              _scheduleRtpInRestart(inst, key, inst.cfg.reconnectMs ?? 10000)
            }
          }
    ...
    socket.on('data', (d) => {
      ...
      while ((nl = buf.indexOf('\n')) >= 0) {
        ...
        if (line.startsWith('[rtp_send] ready')) {
          ...
          if (inst.cfg.target?.host) socket.write(`add ${inst.cfg.target.host} ${inst.cfg.target.port}\n`)
        } else if (line.startsWith('stats ')) {
          const m = line.match(/targets=(\d+)\s+codec=(\S+)\s+bitrateKbps=(\d+)\s+bytesSent=(\d+)/)
          if (m) inst.stats = { connected: Number(m[1]) > 0, codec: m[2], bitrateKbps: Number(m[3]), bytesSent: Number(m[4]) }
        } else {
...
async function _launchRtpOut(cfg) {
  ...
  const inst = {
    ...
    stats: { connected: false, codec: cfg.codec ?? 'raw', bitrateKbps: 0, bytesSent: 0 },
  }
  streamInstances.set(key, inst)

  if (!cfg.target?.host || cfg.enabled === false) { inst.ready = true; return }
...
export function setRtpOutTarget(client, host, port) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_out') throw new Error(`rtp_out ${client} not found`)
  if (inst.cfg.target?.host) _sockWrite(inst.socket, `remove ${inst.cfg.target.host} ${inst.cfg.target.port}`)
  inst.cfg.target = { host, port }
  _sockWrite(inst.socket, `add ${host} ${port}`)
  saveAudioConfig(inst.cfg)
  logger.info('[gst] rtp_out %s set target %s:%d', client, host, port)
}

export function clearRtpOutTarget(client) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_out') throw new Error(`rtp_out ${client} not found`)
  if (inst.cfg.target?.host) {
    _sockWrite(inst.socket, `remove ${inst.cfg.target.host} ${inst.cfg.target.port}`)
    logger.info('[gst] rtp_out %s clear target %s:%d', client, inst.cfg.target.host, inst.cfg.target.port)
  }
  inst.cfg.target = null
  saveAudioConfig(inst.cfg)
  if (inst.server) {
...
export function setRtpOutCodec(client, codec, bitrate) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_out') throw new Error(`rtp_out ${client} not found`)
  const validCodec   = ['mp3', 'raw', 'opus'].includes(codec) ? codec : inst.cfg.codec
...
export function getRtpStreamDetail(client) {
  ...
  return {
    ...
    codec: inst.cfg.codec ?? null, bitrate: inst.cfg.bitrate,
    port: inst.cfg.port, address: inst.cfg.address ?? '0.0.0.0',
    bufferMs: inst.cfg.bufferMs ?? 100, sampleRate: inst.cfg.sampleRate ?? 48000,
    target: inst.cfg.target ?? null, channels: inst.cfg.channels ?? 2,
    stats: inst.stats ? { ...inst.stats } : undefined,
    bufStats: getRtpBufStats(client) ?? undefined,
  }
}

function normalizeRtpInCodec(codec) {
  ...
  switch (codec.toLowerCase()) {
    ...
    default:     return null  /* 알 수 없는 값 → 자동 감지 */
  }
}

export async function setRtpInFormat(client, codec, sampleRate) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_in') throw new Error(`rtp_in ${client} not found`)
  if (codec !== undefined) inst.cfg.codec = normalizeRtpInCodec(codec)
  if (sampleRate !== undefined) inst.cfg.sampleRate = Number(sampleRate)
  saveAudioConfig(inst.cfg)
  if (inst.shouldRun) {
  ...
  logger.info('[gst] rtp_in %s format: codec=%s rate=%d', client, inst.cfg.codec ?? 'auto', inst.cfg.sampleRate ?? 48000)
}

export async function setRtpOutRate(client, sampleRate) {
  const inst = streamInstances.get(client)
  if (!inst || inst.cfg.type !== 'rtp_out') throw new Error(`rtp_out ${client} not found`)
  inst.cfg.sampleRate = Number(sampleRate)
  saveAudioConfig(inst.cfg)
  if (inst.shouldRun) {
    stopRtpStream(client, { silent: true })
    await new Promise(r => setTimeout(r, 200))
  ...
  logger.info('[gst] rtp_out %s sampleRate: %d', client, inst.cfg.sampleRate)
}
...
export function updateRtpOutConfig(client, updates) {
  ...
  if ((updates.codec !== undefined || updates.bitrate !== undefined) && inst.socket)
    _sockWrite(inst.socket, `codec ${inst.cfg.codec ?? 'raw'} ${inst.cfg.bitrate ?? 320}`)
  saveAudioConfig(inst.cfg)
  logger.info('[gst] rtp_out %s config saved (restart to apply)', client)
}

export function stopRtpStream(client, { silent = false } = {}) {
  const inst = streamInstances.get(client)
  ...
  if (inst.cfg.type === 'rtp_out') {
    ...
    inst.socket?.destroy(); inst.socket = null
    inst.server?.close(); inst.server = null
    inst.stats = { connected: false, codec: inst.cfg.codec ?? 'raw', bitrateKbps: 0, bytesSent: 0 }
  }
  if (!silent) streamEvents.emit('state:changed')
}
...
export async function startRtpStream(client) {
  ...
  if (inst.cfg.type === 'rtp_in') {
    ...
    sendToEngine(`rtp_in add ${client} ${ch} ${chStart} bufMs=${inst.cfg.bufferMs ?? 100}`)
  } else if (inst.cfg.type === 'rtp_out') {
    if (!inst.cfg.target?.host) return
    const ch      = inst.cfg.channels ?? 2
...
export function getRtpStreamStatus() {
  return Array.from(streamInstances.entries()).map(([key, inst]) => ({
    ...
    protocol: inst.cfg.protocol ?? 'rtp',
    codec: inst.cfg.codec ?? null,
    bitrate: inst.cfg.bitrate ?? null,
    port: inst.cfg.port ?? null,
    ...
    target: inst.cfg.target ?? null,
    stats: inst.stats ? { ...inst.stats } : undefined,
    bufStats: getRtpBufStats(key) ?? undefined,
  }))
}
...
export async function createRtpStream(cfg) {
  const key = cfg.client
  if (!key) throw new Error('client required')
  if (streamInstances.has(key)) throw new Error(`stream ${key} already exists`)
  if (cfg.type !== 'rtp_in' && cfg.type !== 'rtp_out') throw new Error('type must be rtp_in or rtp_out')
  const fullCfg = { ...cfg, enabled: cfg.enabled !== false }
  addRtpStreamChannels(fullCfg)
  saveAudioConfig(fullCfg)
  if (fullCfg.enabled) {
    if (cfg.type === 'rtp_in')       await _launchRtpIn(fullCfg)
    else if (cfg.type === 'rtp_out') await _launchRtpOut(fullCfg)
  }
  streamEvents.emit('state:changed')
  logger.info('[rtp] created stream %s type=%s', key, cfg.type)
}

export function deleteRtpStream(client) {
  if (streamInstances.has(client)) {
    try { stopRtpStream(client) } catch { /* already stopped */ }
    streamInstances.delete(client)
  }
  removeRtpStreamChannels(client)
  deleteAudioConfig(client)
  streamEvents.emit('state:changed')
  logger.info('[rtp] deleted stream %s', client)
}
