...
const RTP_CFG_KEYS = [
  ...
  'sampleRate', 'bufferMs', 'codec', 'bitrate', 'target', 'enabled', 'address',
]

export function saveAudioConfig(updatedCfg) {
  const raw = getConfig()
  if (!raw.rtp_streams) raw.rtp_streams = []
  const idx = raw.rtp_streams.findIndex((s) => s.client === updatedCfg.client)
  if (idx >= 0) {
    RTP_CFG_KEYS.forEach((k) => {
      if (updatedCfg[k] !== undefined) raw.rtp_streams[idx][k] = updatedCfg[k]
    })
  } else {
    const entry = {}
    RTP_CFG_KEYS.forEach((k) => { if (updatedCfg[k] !== undefined) entry[k] = updatedCfg[k] })
    raw.rtp_streams.push(entry)
  }
  saveConfig()
}

export function deleteAudioConfig(client) {
  const raw = getConfig()
  if (!raw.rtp_streams) return
  const idx = raw.rtp_streams.findIndex((s) => s.client === client)
  if (idx >= 0) raw.rtp_streams.splice(idx, 1)
  saveConfig()
}
