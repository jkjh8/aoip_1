...
const _netCfg = getSystemConfig().network ?? {};
const NETWORK_FILE   = _netCfg.configFile ?? '/etc/systemd/network/20-wired.network';
const DEFAULT_IFACE  = _netCfg.interface  ?? 'eth0';
 ...
 */
export function getNetworkInfo(iface = DEFAULT_IFACE) {
 ...
 */
export async function setStaticIp({ iface = DEFAULT_IFACE, ip, subnet = '255.255.255.0', gateway, dns = '8.8.8.8' }) {
 ...
 * @param {string} iface
 */
export async function setDhcp(iface = DEFAULT_IFACE) {
  const content = `[Match]\nName=${iface}\n\n[Network]\nDHCP=yes\n`;
  await _writeNetworkFile(content);
