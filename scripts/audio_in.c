 ...
 *
 * 수정 사항:
 *   1. anti-windup 로직 수정 (적분항 clamp 방향 버그 수정)
 *   2. input_need > avail 시 SRC 건너뜀 (SRC state 보호)
 *   3. fill_ratio 기준을 TARGET_FILL_FRAMES 명시적 지정으로 변경
 *
...
#define MAX_CH              8
#define PREBUF_FRAMES       256         /* ~5 ms at 48 kHz */
...
static int process_cb(jack_nframes_t nframes, void *arg)
{
    ...
    /* 초기 프리버퍼: PREBUF_FRAMES 이상 쌓이면 시작 */
    if (!g_prebuf) {
    ...
    /* ── 드리프트 보정: PI 제어 ──
     *
     * 수정 1: fill_ratio 대신 TARGET_FILL_FRAMES 기준 명시적 오차 계산
     *         → ring_frames 크기에 무관한 안정적 제어
     */
    ...
    /* ── Anti-windup 수정 ──
     *
     * 수정 2 (버그 수정): 클램프 시 integ를 현재 ratio가 한계값이 되는
     *                     값으로 역산하여 고정.
     *                     이전 코드의 "integ -= error" 방향 오류 제거.
     */
    ...
    /* ── SRC 처리 ──
     *
     * 수정 3 (버그 수정): input_need > avail 이면 SRC 건너뜀.
     *                     이전 코드의 input_need = avail 강제 clamp는
     *                     SRC 내부 필터 상태를 깨뜨려 노이즈를 유발.
     */
    int input_need = (int)ceil((double)nframes / g_ratio) + 2;
    if (input_need > avail) {
        /* 가용 샘플 부족 — 무음 유지, rp 이동 없음 */
        return 0;
    }

    /* ring에서 input_need 프레임 복사 (rp 아직 이동 안 함) */
...
static void *alsa_thread(void *arg)
{
    ...
    if (!pcm) {
        fprintf(stderr, "[audio_in] cannot open %s\n", g_dev);
        g_quit = 1;
        return NULL;
    }
    ...
    if (!ibuf || !fbuf) {
        fprintf(stderr, "[audio_in] buffer alloc failed\n");
        g_quit = 1;
        free(ibuf); free(fbuf);
        return NULL;
    }
    ...
    while (!g_quit) {
        ...
        if (n == -ESTRPIPE) {
            while (!g_quit && snd_pcm_resume(pcm) == -EAGAIN)
                usleep(10000);
            snd_pcm_prepare(pcm);
            continue;
        }
        if (n < 0) {
            ...
            while (!g_quit) {
                ...
                if (pcm) {
                    fprintf(stderr, "[audio_in] device reopened\n");
                    break;
                }
            }
            continue;
...
int main(int argc, char *argv[])
{
    if (argc < 7) {
        fprintf(stderr,
            "Usage: audio_in <client> <device> <rate> <period>"
    ...
    if (g_ch < 1 || g_ch > MAX_CH) {
        fprintf(stderr, "[audio_in] channels out of range (1-%d)\n", MAX_CH);
        return 1;
    }

    g_ring = calloc((size_t)(g_ring_frames * MAX_CH), sizeof(float));
    if (!g_ring) {
        fprintf(stderr, "[audio_in] ring alloc failed\n");
        return 1;
    }
    ...
    if (!g_jack) {
        fprintf(stderr, "[audio_in] JACK connect failed\n");
        return 1;
    }
